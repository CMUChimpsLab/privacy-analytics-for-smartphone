...
async function searchAppsQuery(client, searchQueryType, searchQueryParameters, callback) {
    ...
    switch (searchQueryType) {
        ...
        case 'TAXONOMY_SEARCH': {
            ...
            const findParams = { taxonomies: { $in: tempTaxonomies } };
            if (category) {
...
async function searchMetaDataQuery(client, callback) {
    const metaData = [];
    const db = client.db(dbName);
    const appsCollection = db.collection('apps');
...
async function getPrivacyRatingDistribution(client, taxonomy, callback) {
    const db = client.db(dbName);
    const appsCollection = db.collection('apps');
